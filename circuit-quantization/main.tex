...
Likewise we can define a capacitance matrix $\mathbf{\tilde{C}}$ such that $C_{ij}$ is the negative of the capacitance between nodes $i$ and $j$ and again the diagonal elements are defined to make each row and column sum to 0. Then we can write the total capacitive energy as
\begin{equation}
...
\begin{equation}
    q_i \equiv \frac{\partial\mathcal{L}(\vec\Phi,\dot{\vec{\Phi}})}{\partial\dot{\vec{\Phi}}} = \mathbf{C}\dot{\vec{}}
\end{equation}
